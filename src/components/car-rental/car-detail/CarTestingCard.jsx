...
const fetchSuggestedData = async (addPackage) => {
  const res = await fetch(`/api/cars/public/suggested-packages?packageId=${addPackage?._id}`);
  const data = await res.json();
  return data;
};
const CarTestingCard = ({ addPackage }) => {
              ...
              {suggestedPackage.map((item, i) => (
                ...
                >
                  ...
                  <div className="px-6 pt-1 pb-3 mb-1">
                    <Link
                      href={"/car-rental/" + item.location.url + "/" + item.pageUrl} 
