...
export default function Gst() {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        await fetch('/api/package-setting/gst', {
            method: 'POST',
    ...
    const handleEdit = async (id) => {
        await fetch('/api/package-setting/gst', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ action: 'edit', gstId: id, gstRate: Number(editGstValue) }),
        });
        setEditGstId(null);
        fetchGstList();
    };
        ...
        <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5 rounded">
                <div className="shadow-[0_0px_10px_-3px_rgba(0,0,0,0.3)] p-4 rounded-md bg-white border-l-2 border-teal-600">
                    <form onSubmit={handleSubmit} className="flex items-end justify-between gap-5">
