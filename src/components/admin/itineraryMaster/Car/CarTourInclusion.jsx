...
const CarTourInclusion = () => {
    ...
    const fetchGroups = async () => {
        try {
            const response = await fetch('/api/cars/package/tour-info/inclusion/get');
    ...
    const handleDelete = async () => {
        if (deleteGroupId) {
            try {
                const response = await fetch('/api/cars/package/tour-info/inclusion/delete', {
                    method: 'POST',
    ...
    const handleSave = async () => {
        const url = isEditing
            ? '/api/cars/package/tour-info/inclusion/edit'
            : '/api/cars/package/tour-info/inclusion/add';
                            ...
                            <div key={group._id} className="flex justify-between items-center px-2 even:bg-slate-100 hover:bg-slate-300 cursor-pointer">
                                ...
                                <div className='flex gap-2'>
                                    {(isEditing && editGroupId === group._id) ? (
                                        ...
                                        <FontAwesomeIcon
                                            icon={faTrash}
                                            oonClick={() => toggleModal(group._id)}
...
export default CarTourInclusion;
