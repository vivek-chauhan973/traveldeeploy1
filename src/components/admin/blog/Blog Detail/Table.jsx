...
const Table = ({blogData ,setActiveTab}) => {
  ...
  useEffect(() => {
    if(blogData){
      setRows(blogData?.table?.tableData || []);
      setColumns(blogData?.table?.tableColumn || []);
      if (columns.length !== 0) {
        setSubmitted(true);
      }
    }
...
console.log("blog table data in detail api is here-----> ",blogData)
  const handleSubmit =async () => {

    const data=await fetch("/api/blog/blogtable",{
        method:"POST",
        headers:{
            "Content-Type":"application/json"
        },
        body:JSON.stringify({rows,columns,blog:blogData?._id})
    })
    if(data?.ok){
  ...
  console.log("submitted data is here -----> ",submittedData)
