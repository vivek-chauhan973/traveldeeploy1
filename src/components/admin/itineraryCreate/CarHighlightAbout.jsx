...
const CarHighlightAbout=({ setActiveTab, itinerary, itineraryInfo, setItineraryInfo, setHighlightDot }) =>{
  ...
  const addSubmitHighlight = async () => {
    try {
      const res = await fetch('/api/cars/package/info/' + (itinerary ? itinerary.id : itineraryInfo.id), {
        method: 'POST',
        headers: {
...
export default CarHighlightAbout;
