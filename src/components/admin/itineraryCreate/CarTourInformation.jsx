...
const CarTourInformation = ({ itinerary, setActiveTab, setTourDot }) => {
    ...
    const fData = useCallback(async () => {
        const response = await fetch('/api/cars/package/tour-information/' + (itinerary ? itinerary.id : 'add-package'));
        const data = await response.json();
        if (response.ok) {
            setActiveTab("Tab8")
            setTourDot(true);
    ...
    useFetchData('/api/cars/package/tour-info/inclusion/get', (data) => setTourInclusion(data.inclusionGroupData));
    useFetchData('/api/cars/package/tour-info/exclusion/get', (data) => setTourExclusion(data.ExclusionGroupData));
    useFetchData('/api/cars/package/tour-info/cancellation/get', (data) => setTourCancellation(data.CancellationGroupData));
    useFetchData('/api/cars/package/tour-info/payment-term/get', (data) => setPaymentTerm(data.PaymentTermGroupData));
    useFetchData('/api/cars/package/tour-info/need-to-know/get', (data) => setNeedToKnow(data.NeedToKnowGroupData));
