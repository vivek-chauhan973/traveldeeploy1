...
useEffect(()=>{
...
},[durationRange?.[0],durationRange?.[1],priceRange?.[0],priceRange?.[1],categoryId?.length])
  // Updated to handle category selection
  const handleCheckboxChange = (event) => {
    const value = event.target.value;
    setCategoryId((prev) => {
      if (prev.includes(value)) {
        // If the value already exists in state, remove it (checkbox unchecked)
        return prev.filter((item) => item !== value);
      } else {
        // If the value doesn't exist in state, add it (checkbox checked)
        return [...prev, value];
      }
    });
  };
  ...
  const handleClearAll = () => {
    ...
    setCategoryId([]); // Reset categoryId on clear all
    setClearAll(true);
    document
      ...
      <div className="bg-white rounded-md sticky top-2">
        ...
        <div>
          ...
          <div className="pr-5 py-2">
            <h3 className="md:text-[16px] text-[14px] font-medium md:my-2 my-1 px-5">
              Package Category
            </h3>
            <div>
              {packageCategory?.map((item) => (
                ...
                >
                  <input
                    ...
                    name={item?.category}
                    value={item?._id}
                    onChange={handleCheckboxChange} // Use the updated function here
                  />
                  <label
