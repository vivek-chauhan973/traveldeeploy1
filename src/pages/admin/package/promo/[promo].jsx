...
export default function PromoManage() {
  ...
  console.log("router is here : ", promo);
  ...
  useEffect(() => {
    if (promo) {
      getPromoData(promo).then((res) => {
        setPromoTxt(res?.data);
        console.log("res------->", res);
      });
    }
  }, [promo]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(
        `/api/public/package-state/${selectedLocation || promo}`,
        {
          method: "POST",
          body: formData,
        }
  ...
  const handlePosterUpload = async () => {
    ...
    try {
      const response = await fetch(
        `/api/public/package-state/poster?id=${selectedLocation || promo}`,
        {
          method: "POST",
          body: formData1,
        }
          ...
          <div>
            <div className="flex flex-col ">
              <div className="flex flex-col sm:flex-row md:items-center gap-2 mb-4 w-full">
                ...
                <select className="mt-1 md:ml-2 h-7  md:w-32 w-full rounded-md outline-none border-slate-500/45 cursor-pointer border text-para">
            ...
            <button
              ...
              className="grow w-full bg-black font-semibold text-white py-3 mt-5 rounded"
            >
              ADD
            </button>
          </div>
