        ...
        switch (req.method) {
            case 'PUT':
                const {priority, name, price, status, location,category,badges ,startcity,uploads, selectedState,
                    selectedCountry,packageRating,PackageIdGenerate } = req.body;
                ...
                updatedPackage = await Package.findByIdAndUpdate(packageId, {priority, name, price, status, location,category,badges,startcity:startcity1,uploads:images,country:selectedCountry,state:selectedState,packageRating,PackageIdGenerate }, { new: true });
