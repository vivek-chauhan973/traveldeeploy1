...
// find By Id And Update
 const packageIds= async (req, res) => {
    try {
        ...
        switch (req.method) {
            case 'PUT':
                const {priority,selectedCarName, name, price, status, location,category,badges ,startcity,uploads, selectedState,
                ...
                updatedPackage = await CarPackage.findByIdAndUpdate(packageId, {priority, name,selectedCarName, price, status, location,category,badges,startcity:startcity1,uploads:images,country:selectedCountry,state:selectedState,packageRating,highlightedPackage }, { new: true });
