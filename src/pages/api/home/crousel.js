...
const uploadDirectory = path.join(process.cwd(), "uploads/Crousel");
  ...
  if (req.method === "POST" || req.method === "PUT") {
    upload.single("file")(req, File, async (err) => {
      ...
      const fileData = req.file && {
        ...
        path: `/api/uploads/Crousel/${req.file.filename}`,
      };
      try {
    ...
    try {
      if (id) {
        const files1 = await Crousel.find({ _id: id });
        return res.status(200).json({ data: files1 });
      }
      const files = await Crousel.find({});
    ...
    try {
      const file = await Crousel.findById({ _id: id });
      if (file) {
        ...
        await Crousel.findByIdAndDelete({ _id: id });
