...
APP.getInitialProps = async (appContext) => {
  ...
  const res = await fetch("http://localhost:3000/api/search-api");
  const data = await res.json();

  try {
    const [data, data1, data2, data3, data4] = await Promise.all([
      fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/home/homefooter`).then(
        (res) => res.json()
      ),
      fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/homefooter`).then((res) =>
        res.json()
      ),
      fetch(
      ...
      fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/search-api`).then((res) =>
        res.json()
      ),
    ]);
    ...
    const citiesRes = await fetch(
      `${process.env.NEXT_PUBLIC_BASE_URL}/api/home/headerCity?id=${firstStateId}`
    );
